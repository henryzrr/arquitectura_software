...
public class Apply implements Command {
    ...
    public void execute(String args) {
        try {
            ...
            Tool tool = toolContainer.getTool(toolName);
            executeCommand(tool,settings.getDir(dirName));
        }catch (Exception e){
            e.printStackTrace();
            System.exit(1);
        }
    ...
    private void executeCommand(Tool tool,String dirPath) {
        try{
            List<String> command = tokenizerCommand(tool.getArgs());
            getAllFiles(command,dirPath,tool.getIn());
